...
4. Update the disk distributer weights for all disks, not just local ones. 

5. If we for real added a disk to LocalDiskServices then send a message `DiskAddedMessage` to all other nodes.
...
6. Each time something is added to `DiskManagerSvc.DiskInfoList` persist the list to `disks.json`

7. On startup read `disks.json` and load the two lists inside `DiskManagerSvc`. Block on startup so that the subsequent `Iam` message has the info it needs.

8. `Iam` contains a `DiskAddedList`
